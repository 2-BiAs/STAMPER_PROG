...
//Setup Boundary And final profile for Ni multipass
plRoundedBound = OffsetPolyline(plRounded, vNormalOffsetRegime, mNickelBuildupComp);
////plRoundedBound = RemoveSharpCorners(plRoundedBound, 45 * (%pi / 180)); 
plRoundedBound = SlicePolyline(plRoundedBound, [-234 -2; -234 0], %T);
plRoundedBound = [-234 -2; plRoundedBound(1)];
plFinalPassRounded = RadOutsideCorners(vPoints, .001, 12);


//plRounded = plNiUnderCut;

plBoundary = [-234 -2; -234 .1; 1 .1];
fZ_Clearance = 1 ;

//DuperPlot(plFinalPassRounded);
//DuperPlot(plRoundedBound);
//DuperPlot(plBoundary);
////
//pause

////For Multipass Roughing Brass Cut
...
//For Multipass Roughing Ni Cut
[listCutPath, listCompedCutPath, iLevelIndices] = NCompMultiOffsetToBnd(listFinalPass, listMPOR, listMPOC_FIN, plBoundary, 1, vORBM_ROUGH, mCCOC_ROUGH);
