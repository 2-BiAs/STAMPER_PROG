...
exec('Test_Setup_Parameters.sce');
...
plNiUnderCut = OffsetPolyline(listFinalPass(1), vNormalOffsetRegime, mNiUndercutComponents)

plBoundWithRad = AddInsideRadSmoothNi(plNiUnderCut, vORBM_ROUGH, mCCOC_ROUGH);

//[listCutPath, listCompedCutPath, iLevelIndices] = NCompMultiOffsetToBnd(list(vPoints), listMPOR, listMPOC_FIN, plBoundary, 1, vORBM_FIN, mCCOC_FIN);
//
//
...
for i=1:length(listFinalPass(:,1))
    mfprintf(fFile, "X%fZ%f", listFinalPass(i,1), listFinalPass(i,2));
    mfprintf(fFile, "   %s\n", sLineEndCode(i));
end
mclose(fFile);
